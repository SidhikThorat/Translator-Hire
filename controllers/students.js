...
module.exports.index = async (req, res) => {
  ...
  try {
    const allStudents = await Student.find(filter); // Search by Filter
    res.render('studentIndex.ejs', { allStudents });
  } catch (error) {
    console.error(error);
    res.status(500).send("Error occurred while searching for Students");
  }
};


module.exports.filterStudents = async (req, res) => {
  ...
  try {
    const filteredStudents = await Listing.find(filter);
    res.render('studentFiltered.ejs', { filteredStudents });
  } catch (error) {
    console.error(error);
    res.status(500).send("Error occurred while filtering Students");
  }
};
...
module.exports.showListing = async (req, res, next) => {
  ...
  const listing = await Student.findById(id).populate("reviews").populate("owner");
  if (listing) {
